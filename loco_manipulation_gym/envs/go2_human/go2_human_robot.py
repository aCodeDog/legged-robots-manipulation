...
from .go2_human_config import Go2HumanRoughCfg
...
class Go2HumanRobot(LeggedRobot):
    cfg: Go2HumanRoughCfg
    ...
    def _init_buffers(self):
        ...
        self.commands_scale = torch.tensor([self.obs_scales.lin_vel, self.obs_scales.lin_vel, self.obs_scales.lin_vel,self.obs_scales.ang_vel], device=self.device, requires_grad=False,) # TODO change this
        self.feet_air_time = torch.zeros(self.num_envs, self.human_leg_indices.shape[0], dtype=torch.float, device=self.device, requires_grad=False)
        self.last_contacts = torch.zeros(self.num_envs, len(self.human_leg_indices), dtype=torch.bool, device=self.device, requires_grad=False)
        self.base_lin_vel = quat_rotate_inverse(self.base_quat, self.root_states[:, 7:10])
        self.base_ang_vel = quat_rotate_inverse(self.base_quat, self.root_states[:, 10:13])
        self.projected_gravity = quat_rotate_inverse(self.base_quat, self.gravity_vec)
        if self.cfg.terrain.measure_heights:
    ...
    def _create_envs(self):
        ...
        human_leg_names =[]
        for name in self.cfg.asset.human_leg_names:
            human_leg_names.extend([s for s in body_names if name in s])
        ...
        self.human_leg_indices = torch.zeros(len(human_leg_names), dtype=torch.long, device=self.device, requires_grad=False)
        for i in range(len(human_leg_names)):
            self.human_leg_indices[i] = self.gym.find_actor_rigid_body_handle(self.envs[0], self.actor_handles[0], human_leg_names[i])
        print("###",human_leg_names)
    ...
    def _reward_tracking_ang_vel(self):
        ...
        ang_vel_error = torch.square(self.commands[:, 3] - self.base_ang_vel[:, 0])
        return torch.exp(-ang_vel_error/self.cfg.rewards.tracking_sigma)

    def _reward_feet_air_time(self):
        # Reward long steps
        # Need to filter the contacts because the contact reporting of PhysX is unreliable on meshes
        contact = self.contact_forces[:, self.human_leg_indices, 2] > 1.
    ...
    def _reward_stumble(self):
        ...
        return torch.any(torch.norm(self.contact_forces[:, self.human_leg_indices, :2], dim=2) >\
             5 *torch.abs(self.contact_forces[:, self.human_leg_indices, 2]), dim=1)
    ...
    def _reward_feet_contact_forces(self):
        ...
        return torch.sum((torch.norm(self.contact_forces[:, self.human_leg_indices, :], dim=-1) -  self.cfg.rewards.max_contact_force).clip(min=0.), dim=1)
    
    def _reward_no_fly(self):
        contacts = self.contact_forces[:, self.human_leg_indices, 2] > 1.
        single_contact = torch.sum(1.*contacts, dim=1)==1
        return 1. * single_contact * (torch.norm(self.commands[:, :3], dim=1) > 0.05)
    def _reward_hip_action(self):
        return torch.abs(self.dof_pos[:,0]) + torch.abs(self.dof_pos[:,3])+ torch.abs(self.dof_pos[:,6])+ torch.abs(self.dof_pos[:,9])

    def _reward_leg_symmetry(self):
        return torch.abs(self.dof_pos[:,6] - self.dof_pos[:,9])
